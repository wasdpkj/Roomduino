...
char urlBuf0[] PROGMEM = "/v1.0/device/xxx/sensor/xxx/";	//yeelink模式开关

char urlBuf1[] PROGMEM = "/v1.0/device/xxx/sensor/xxx/";	//节点
char urlBuf2[] PROGMEM = "/v1.0/device/xxx/sensor/xxx/";	//节点
char urlBuf3[] PROGMEM = "/v1.0/device/xxx/sensor/xxx/";	//节点
char urlBuf4[] PROGMEM = "/v1.0/device/xxx/sensor/xxx/";	//节点

char apiKey[] PROGMEM = "U-ApiKey: xxx";					//APIKEY
...
RF24 radio(A5, A4); //设置24L01的CE和CSN引脚
  ...
  if(NET_WEBSITE_sta)
  {
    if (millis() - NET_YEE_mil > NET_YEE_request)    //采集节点时间
    {
      ...
      case 0:
        ...
        if(MOD != yee[0] && NET_YEE_sw)    //网络模式判断激活时，先执行这个，把网络模式状态推出去
        {
          if(!MOD)
...
void menu(void)
{
  ...
  if(MENU_xuan0sta)								//进入初级菜单后
  {
    ...
    u8g.drawFrame(0, 0 + (8 * MENU_xuan0), 83, 8);		//选框
...
void vomenu()
{
  ...
  if(MENU_xuan0sta)							//进入初级菜单后
  {
    ...
    MENU_xuan0 = pkj.VOLtiaobian(MENU_xuan0, 1, 5);
